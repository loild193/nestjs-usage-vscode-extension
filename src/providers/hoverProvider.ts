...
export class NestJSHoverProvider implements vscode.HoverProvider {
    ...
    async provideHover(
        ...
        const keywords = ['if', 'else', 'for', 'while', 'return', 'const', 'let', 'var',
            'function', 'class', 'import', 'export', 'from', 'async', 'await',
            'true', 'false', 'null', 'undefined', 'this', 'new', 'throw', 'try',
        ...
        if (allUsages.length > 0) {
            ...
            for (const usage of displayUsages) {
                ...
                markdownContent.appendMarkdown(
                    `- [${relativePath}:${line}](${usageUri}) — \`${preview}\`\n`
                );
            }
            ...
            if (hasMore) {
                ...
                const commandUri = vscode.Uri.parse(
                    `command:nestjs-usage.showAllUsages`
                );
                markdownContent.appendMarkdown(
                    `[📋 Show all ${allUsages.length} usages](${commandUri})`
                );
            }
        } else {
